...
Важное отличие от прежней версии: кеш держит только ОКНО вокруг
сегодняшнего дня (по умолчанию −30 / +60 дней). Раньше он запрашивал
всю историю уроков целиком каждые 2 минуты — объём рос без ограничений
и упирался в лимит 5 запросов/сек AlfaCRM.
"""
...
class LessonCache:
    ...
    async def refresh(self) -> None:
        ...
        async with self._lock:
            try:
                lessons = await self.alfacrm.get_lessons(date_from=date_from, date_to=date_to)
                lessons = [l for l in lessons if l.get("id") is not None]

                by_teacher: Dict[int, List[Dict[str, Any]]] = {}
                ...
                if not lessons:
                    logger.warning(
                        "⚠️ Кеш пуст. Проверьте branch_id, доступ пользователя API к v2api "
                        "и наличие уроков в окне."
                    )
    ...
    def get_lessons(
        ...
        if date_from or date_to:
            # Раньше фильтр смотрел только на поле `date`; уроки, у которых
            # дата есть лишь внутри time_from, молча пропадали из расписания.
    ...
    async def patch_lesson(self, lesson_id: int, updates: Dict[str, Any]) -> None:
        """
