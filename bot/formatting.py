...
Модуль зависит только от stdlib и settings.py (который тоже не тянет
ничего проектного), поэтому не может сломать порядок импортов.
...
Основные точки входа:
    ...
    answer_blocks / send_blocks         отправка с защитой от флуд-лимита
"""
...
# Статусы берём из settings, чтобы они не расходились с alfacrm_client.
...
def esc(value: Any) -> str:
    """Экранирует текст из CRM и от пользователей — он уходит в parse_mode=HTML."""
    return escape(str(value if value is not None else ""), quote=False)


# Короткий алиас для внутреннего использования (обратная совместимость).
_esc = esc
...
def fmt_db_time(value: Any) -> str:
    ...
    CURRENT_TIMESTAMP в SQLite всегда UTC, поэтому «Создана: …» у заявок
    показывалась на 3 часа раньше реального московского времени.
    """
...
# ==================== ОТПРАВКА СООБЩЕНИЙ ====================
#
# Telegram: ~1 сообщение в секунду в один чат, ~30 суммарно.
# Раньше расписание на месяц уходило сотней сообщений подряд без пауз —
# это гарантированный TelegramRetryAfter и потерянные сообщения.

async def safe_call(factory, *, attempts: int = 3):
    """
    Выполняет отправку, уважая flood-лимит.
    ...
    for attempt in range(attempts):
        ...
        except Exception as e:  # aiogram.TelegramRetryAfter и совместимые
...
async def answer_blocks(message: Any, blocks: Sequence[str]) -> None:
    """Отправить список блоков ответом на сообщение (aiogram Message)."""
...
async def send_blocks(bot: Any, chat_id: int, blocks: Sequence[str]) -> None:
    """Отправить список блоков в чат по chat_id (для планировщика и рассылок)."""
    for i, block in enumerate(blocks):
        if i:
            await asyncio.sleep(settings.TG_SEND_DELAY)
        await safe_call(lambda b=block: bot.send_message(chat_id, b, parse_mode="HTML"))
